...
router.put("/:show_id", async (req, res) => {
  try {
    const updatedShow = await TVShow.findOneAndUpdate({ show_id: req.params.show_id }, req.body, { new: true });
    if (!updatedShow) {
...
router.delete("/:show_id", async (req, res) => {
  try {
    const deletedShow = await TVShow.findOneAndDelete({ show_id: req.params.show_id });
    if (!deletedShow) {
      return res.status(404).json({ message: "TV show not found" });
    }
    res.status(200).json({ message: "TV show successfully deleted" });
  } catch (err) {
